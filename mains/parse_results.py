...
def get_stats(data: dict) -> dict:
    ...
    stats['Simulations'] = calc_stats(data['Simulations'])
...
def get_data_for_iterations(data: dict, iter: int) -> dict:
    ...
    #new_data['Exploration Weight'] = [float(data[n]['Exploration Weight']) for n in data if int(data[n]['Iterations']) == iter]
    ...
    new_data['Simulations'] = [int(data[n]['Simulations']) for n in data if int(data[n]['Iterations']) == iter]
...
def draw_scatter_plot_nodes(fig, data: dict):
    fig.set_title("Nodes in the tree search")
    fig.set_xlabel("Iterations")
...
def draw_scatter_plot_simulations(fig, data: dict):
    fig.set_title("Simulations (configs evaluated)")
    fig.set_xlabel("Iterations")
    fig.set_ylabel("Simulations")
    ...
    x1 = list(data.keys())
    x1.sort()
    y1 = [data[iter]['stats']['Simulations']['median'] for iter in x1]
...
def plot_results(data):
    ...
    draw_scatter_plot_simulations(axs[1,1], data)
    draw_scatter_plot_times(axs[2,0], data)
    fig.tight_layout()
    plt.show()
...
def parse_results(filespath: List['str'], output_file: str = OUTPUT_SUMMARY_FILE):
    ...
    with open(output_file, 'w+') as file:
        file.write("Iterations, Runs, Valid Configs, Invalid configs, Features min, Features max, Features median, Features mean, Features std, Rewards min, Rewards max, Rewards median, Rewards mean, Rewards std, Nodes min, Nodes max, Nodes median, Nodes mean, Nodes std, Times min, Times max, Times median, Times mean, Times std, Simulations min, Simulations max, Simulations median, Simulations mean, Simulations std\n")
