...
class ConfigurationStateRelations(State):
    ...
    def reward(self) -> float:
        ...
        if linux_feature in self.configuration.elements:
            packages_with_errors = [f for f in self.configuration.elements if f.name in PACKAGES_WITH_ERRORS_IN_LINUX and self.configuration.elements[f]]
        elif win_feature in self.configuration.elements:
            packages_with_errors = [f for f in self.configuration.elements if f.name in PACKAGES_WITH_ERRORS_IN_WIN and self.configuration.elements[f]]

        n = len(self.feature_model.get_features()) - len(self.configuration.elements)
        valid = 1 if self.is_valid_configuration else -1
